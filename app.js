...
const username = encodeURIComponent("yetesoh768");
const password = encodeURIComponent("Asdf@12345");
const databaseName = "projective_test";
const mongoDBUrl = `mongodb+srv://${username}:${password}@cluster0.oywxkfe.mongodb.net/${databaseName}?retryWrites=true&w=majority`;
mongoose
  .connect(mongoDBUrl, {
...
app.post("/predict", upload.single("image"), async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Label for the image : " + predictedLabel });
  } catch (error) {
...
app.post("/api/uploadTrainData", upload.array("images"), async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to save training data", message: error });
  }
});
...
async function trainModel(res) {
  try {
    const trainData = await TrainData.find();

    if (trainData.length === 0) {
      console.log("No training data available");
      return res.json({ error: true, message: "No training data available" });
    }
    ...
    if (labelSize < 2) {
      ...
      return res.json({
        error: true,
        message: "Insufficient number of unique labels",
      });
    }
    ...
    const xsTrain = tf.tensor2d(
      processedDataset.map((data) => {
        const inputFeature = Array.isArray(data.inputFeature)
          ? data.inputFeature
          : [data.inputFeature];
...
async function processUploadData(imageData) {
  try {
    ...
    const processedImageData = await processedImage.getBufferAsync(
      Jimp.MIME_JPEG
    );
...
async function predictImageData(imageData) {
  try {
    ...
    const inputFeature = Array.isArray(processedImage)
      ? processedImage
      : [processedImage];
    ...
    const labels =
      uniqueLabels.length > 0
        ? uniqueLabels
        : ["Person Under The Rain", "Family", "Human Figure"]; // Replace with your own labels
